...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x_shape = x.size()
        # b, d, h, w, c = x_shape
        # pad_input = (h < 3) or (w < 3) or (d < 3)
        # print(111, x.shape)
        # if pad_input:
        #     x = F.pad(x, (0, 0, 0, 3 - w, 0, 3 - h, 0, 3 - d))
        #     print(222, x.shape)
        x = self.expand(x)
        # print(x.shape)
        if len(x_shape) == 5:
...
class SwinUnet(nn.Module):
    """
    Conventional 3D version of the Swin-Unet model based on: "Swin-Unet: Unet-like Pure Transformer for Medical Image Segmentation
    <https://arxiv.org/pdf/2105.05537.pdf>"
    ...
    def __init__(
        ...
        self.up = FinalPatchExpand_X4(dim=self.embed_dim, dim_scale=patch_size, spatial_dims=spatial_dims)
    ...
    def forward(self, x, normalize=True):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = rearrange(x, "b c d h w -> b d h w c")
                x = layer_up(x.contiguous())
                x = rearrange(x, "b d h w c -> b c d h w")
                x = self.proj_out(x, normalize)
            else:
                x = rearrange(x, "b c d h w -> b d h w c")
                x = torch.cat([x, rearrange(x_downsample[self.num_layers - inx - 1], "b c d h w -> b d h w c")], -1)
