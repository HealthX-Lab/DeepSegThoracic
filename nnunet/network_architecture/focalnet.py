...
# edited by Amirhossein Rasoulian (ah.rasoulian@gmai.com) to adjust it to 3D setup
# --------------------------------------------------------
...
class FocalModulation(nn.Module):
    ...
    def forward(self, x):
        """
        Args:
            x: input features with shape of (B, H, W, D, C)
        """
        C = x.shape[-1]

        # pre linear projection
        x = self.f(x).permute(0, 4, 1, 2, 3).contiguous()
