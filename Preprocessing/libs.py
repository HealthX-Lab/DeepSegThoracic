...
def combine_masks_to_multilabel_file(masks_dir, multilabel_file):
    ...
    masks = class_map["totalUpper"].values()
...
def combine_masks_to_multilabel_file_groups(masks_dir, multilabel_file, class_type):
    """
    Generate one multilabel nifti file from a directory of single binary masks of each class.
    This multilabel file is needed to train a nnU-Net.
    ...
    if class_type == "ribs":
        ...
        masks = list(class_map_5_parts["class_map_part_organs"].values())
        ...
        masks = class_map["totalUpper"].values()
    masks_dir = Path(masks_dir)
    ref_img = nib.load(masks_dir / f"{masks[0]}.nii.gz")
...
def combine_masks(mask_dir, output, class_type):
    ...
    output: output path
    class_type: ribs | organs | vertebrae | cardiac | muscles | vertebrae_ribs | lung | heart
    """
    if class_type == "ribs":
        masks = list(class_map_5_parts["class_map_part_ribs"].values())
    elif class_type == "organs":
